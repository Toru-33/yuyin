...
class Ws_Param(object):
    # 初始化
    def __init__(self, APPID, APIKey, APISecret, Text, voice_type="x4_EnUs_Laura_education", speed=50, volume=50):
        self.APPID = APPID
        self.APIKey = APIKey
...
def run(video_file, subtitle_file, output_path, voice_type="x4_EnUs_Laura_education", speed=100, volume=80):
    ...
    if output_path and output_path.endswith('.mp4'):
        ...
        final_output_path = os.path.join(output_dir, f"{base_name}_en_to_en.mp4").replace("\\", "/")
    ...
    for i in range(len(segments)):
        try:
            os.remove(f'{tmpname}{i}.wav')
            os.remove(f'{changeSpeedTmpname}{i}.wav')
        except FileNotFoundError:
            pass

    # 合并视频和音频
    try:
        subprocess.run(['ffmpeg', '-y', '-i', video_file, '-i', 'newOutputEn.wav', '-c', 'copy', final_output_path],
                       capture_output=True, text=True, encoding='utf-8', errors='replace')
        print(f"视频合成成功: {final_output_path}")
        
        # 清理临时音频文件
        try:
            os.remove('newOutputEn.wav')
            if os.path.exists('demo.mp3'):
                os.remove('demo.mp3')
        except FileNotFoundError:
            pass
