...
def speed_change(audio_path, start_time, end_time):
    # 读取音频文件
    audio = AudioSegment.from_file(audio_path, format="wav")
    # audio = audio.set_channels(1)
    reader = audiotsm.io.wav.WavReader(audio_path)

    writer = audiotsm.io.wav.WavWriter("output_speed_en" + str(i) + ".wav", 1, 16000)  # 1：单通道。  16000：采样率
...
if __name__ == '__main__':
    video_file = r'D:\PyCharmProfessional\PyCharm\Project\YuYinTiHuan\videoWithoutAudio.mp4'
    #
    # # 设置字幕文件路径
    subtitle_file = r'D:\PyCharmProfessional\PyCharm\Project\YuYinTiHuan\Cn.srt'

    syntheticSpeechTranslateToEn.run(video_file, subtitle_file)
    ...
    # tmpname = 'en_audio_segment_'
    # changeSpeedTmpname = 'output_speed_en'
    ...
    #     command = 'ffmpeg ' + '-y -i ' + 'demo.mp3 en_audio_segment_' + str(i) + '.wav'
    #     os.system(command)
    #     # 合并所有语音片段
    #     speed_change(tmpname + str(i) + '.wav', start_time, end_time)
    ...
    # merged_audio.export(f"./newOutputEn.wav", format="wav")
    #
    #
    #
    # # 删除所有语音片段
    # # for i in range(len(segments)):
    # #     os.remove(f'audio_segment_{i}.mp3')
    # #
    #
    # # 合并视频和音频
    # subprocess.run(['ffmpeg', '-i', video_file, '-i', 'newOutputEn.wav', '-c', 'copy', 'outputEn.mp4'])
    #
